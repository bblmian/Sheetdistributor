...
async function setDataRange(): Promise<void> {
  try {
    // 立即显示反馈，避免用户以为没有反应
    showMessage("正在设置数据区域...", false);
    appendDebugLog("开始执行 setDataRange");
    ...
    await Excel.run(async (context) => {
      ...
      appendDebugLog(`选中区域原始引用: ${address}`);
      ...
      // 先尝试删除已存在的（如果存在）
      try {
        // 使用 getItem 而不是 getItemOrNullObject，因为我们在 catch 中处理
        const existingItem = namedItems.getItem(CFG_DATA_RANGE_NAME);
        // 加载属性以确保对象存在
        existingItem.load("name");
        await context.sync();

        appendDebugLog(`找到旧名称，准备删除: ${CFG_DATA_RANGE_NAME}`);
        existingItem.delete();
        await context.sync();
        appendDebugLog(`已删除旧定义的名称`);
      } catch (error) {
        // ItemNotFound 是预期的，其他错误则记录
        if (error.code !== "ItemNotFound") {
          appendDebugLog(`检查旧名称时遇到非404错误: ${error.code} - ${error.message}`);
        } else {
          appendDebugLog(`旧名称不存在 (ItemNotFound)，无需删除`);
        }
      }
